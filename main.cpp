...
MyOS::MyOS(QWidget *parent):QMainWindow(parent), ui(new Ui::MyOS) {
    ui->setupUi(this);
    diskSpace(); specs(); graph(); fileExplorer(); settings(); taskManager(); cpuLoad(); saveShortcuts();
...
void MyOS::specs() {
    ...
    QString getGpuType = gpuType(); getGpuType.remove("(R)"); ui->label_15->setText(getGpuType);
}
void MyOS::graph() {
    QLineSeries *series = new QLineSeries();
    QTimer *timer = new QTimer(); timer->start(1000);
    int i = 0; bool isRAMUsage = false; bool isCPULoad = true;
    connect(timer, &QTimer::timeout, this, [this, series, &i, &isRAMUsage]() {
        if (i < 61) { if (isRAMUsage == true) {
                MEMORYSTATUSEX status; status.dwLength = sizeof(status); GlobalMemoryStatusEx(&status);
    ...
    connect(timer, &QTimer::timeout, this, [this, &i, &isCPULoad, &isRAMUsage]() {
        if (i < 61 && isCPULoad == true) {
            isRAMUsage = false; float getCpuLoad = cpuLoad();
            ui->label_29->setText(QString("CPU usage and load in the last minute: (Usage: X%, Load: %1%)").arg(QString::number(getCpuLoad, 'f', 1)));
        }
    });
    ...
    connect(cpuButton, &QPushButton::clicked, [series, &i, &isCPULoad, &isRAMUsage]{i = 0; series->clear(); isRAMUsage = false; isCPULoad = true;});
    connect(gpuButton, &QPushButton::clicked, [series, this, &i, &isCPULoad, &isRAMUsage]{i = 0; series->clear(); isRAMUsage = false; isCPULoad = false; ui->label_29->setText("GPU usage in the last minute");});
    connect(ramButton, &QPushButton::clicked, [series, &i, &isRAMUsage, &isCPULoad](bool) {i = 0; series->clear(); isRAMUsage = true; isCPULoad = false;});
}
...
void MyOS::fileExplorer() {
    ...
    connect(ui->addButton, &QPushButton::clicked, this, [=, i = 1]() mutable {
        QWidget *newTab = new QWidget; ui->tabWidget->insertTab(i, newTab, tr("New Tab")); QPushButton *closeButton = new QPushButton("", newTab);
        closeButton->setFixedSize(16, 16); closeButton->setStyleSheet("background-color: transparent; border: none;");
        ...
        connect(closeButton, &QPushButton::clicked, this, [=]() mutable {
            ui->tabWidget->removeTab(ui->tabWidget->indexOf(newTab)); i--; ui->addButton->move(ui->addButton->pos().x() - 102, ui->addButton->pos().y());
        }); ui->tabWidget->setCurrentIndex(i + 1); ui->addButton->move(ui->addButton->pos().x() + 102, ui->addButton->pos().y());
    });
...
void rLineEdits(QLineEdit* lineEdit, const QString& locName, QLineEdit* lineEdit2, const QStandardPaths::StandardLocation& standardLoc) {QDirIterator it(QStandardPaths::writableLocation(standardLoc)); lineEdit->setText(locName); lineEdit2->setText(it.path());}
void MyOS::settings() {
    ...
    connect(ui->clearTrashButton_2, &QPushButton::clicked, this, [this]() {
        ...
        rLineEdits(ui->lineEdit, "Pictures", ui->lineEdit_2, QStandardPaths::PicturesLocation); rLineEdits(ui->lineEdit_3, "Documents", ui->lineEdit_4, QStandardPaths::DocumentsLocation);
        rLineEdits(ui->lineEdit_6, "Downloads", ui->lineEdit_5, QStandardPaths::DownloadLocation);
    });
