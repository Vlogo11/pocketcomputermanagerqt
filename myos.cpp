...
void MyOS::diskSpace() {
    ...
    connect(ui->menuStorage, &QMenu::aboutToShow, [this]() {ui->diskInfo->show();ui->specs->hide();ui->tasks->hide();ui->settings->hide();});
}
void MyOS::specs() {
    ...
    connect(ui->menuSpecs, &QMenu::aboutToShow, [this]() {ui->diskInfo->hide();ui->specs->show();ui->tasks->hide();ui->specs->move(5, 5);ui->settings->hide();});
...
void MyOS::graph() {
    ...
    connect(timer, &QTimer::timeout, [this, series, &i, &isRAMUsage]() {
        if (i < 61) {
            if (isRAMUsage == true) {
                MEMORYSTATUSEX status; status.dwLength = sizeof(status); GlobalMemoryStatusEx(&status);
    ...
    connect(timer, &QTimer::timeout, [this, &i, &isCPULoad, &isRAMUsage]() {
...
void MyOS::fileExplorer() {
    ...
    connect(ui->addButton, &QPushButton::clicked, [=, this]() mutable { i++;
        ...
        connect(closeButton, &QPushButton::clicked, [=, this]() mutable {
            int tabIndex = ui->tabWidget->indexOf(newTab); ui->tabWidget->removeTab(tabIndex); i--;
            QPoint addButtonPos = ui->addButton->pos(); ui->addButton->move(addButtonPos.x() - 102, addButtonPos.y());
        });
        ui->tabWidget->setCurrentIndex(i + 1); QPoint addButtonPos = ui->addButton->pos(); ui->addButton->move(addButtonPos.x() + 102, addButtonPos.y());
    });
    ...
    connect(renameAction, &QAction::triggered, [=, this]() {
        ...
        connect(lineEdit, &QLineEdit::editingFinished, [=]() {QFile::rename(model->filePath(index), QDir(model->fileInfo(index).path()).filePath(lineEdit->text()));});
    });
    ui->usernameLineEdit_2->setStyleSheet("border: 1px solid #E5E5E5;");
    connect(ui->usernameLineEdit_2, &QLineEdit::editingFinished, [=, this]() {
        QString newPath = ui->usernameLineEdit_2->text(); QFileInfo fileInfo(newPath);
    ...
    connect(ui->usernameLineEdit_3, &QLineEdit::returnPressed, [=, this]() {
        ...
        QThreadPool::globalInstance()->start([=]() {
            ...
            QStringList results; QMutex resultsMutex;
    ...
    connect(ui->pushButton, &QPushButton::clicked, [=, this]() {
        if (searchModel->rowCount() > 0) {
            ...
            ui->usernameLineEdit_2->setText(model->rootPath()); model->setRootPath(model->rootPath());
        } else {
...
void MyOS::settings() {
    ...
    connect(ui->menuSettings, &QMenu::aboutToShow, [this]() {ui->diskInfo->hide();ui->specs->hide();ui->tasks->hide();ui->settings->show();ui->settings->move(5, 5);});
    ...
    connect(ui->clearTrashButton_2, &QPushButton::clicked, [this]() {
